...
const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    if (!loading && authenticated && isAdmin) refreshAdminData().catch((err) => setError(err.message));
  ...
  async function handleSaveUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const payload = { ...userForm, email: userForm.email.trim().toLowerCase(), password: userForm.password.trim() };
  ...
  async function handleDeleteProject(project: AdminProject) {
    const confirmed = window.confirm(`Excluir o projeto \"${project.name}\"? Isso remove o cadastro e os vínculos de acesso, mas não apaga o arquivo no Dropbox.`);
    if (!confirmed) return;
    ...
    try {
      await fetch(`/api/admin/projects/${project.id}`, { method: "DELETE", credentials: "include", headers: { Accept: "application/json" } }).then(readJson);
  ...
  async function handleDeleteUser(targetUser: AdminUser) {
    const confirmed = window.confirm(`Excluir o usuário \"${targetUser.email}\"? Isso remove a conta, sessões e vínculos de acesso.`);
    if (!confirmed) return;
    ...
    try {
      await fetch(`/api/admin/users/${targetUser.id}`, { method: "DELETE", credentials: "include", headers: { Accept: "application/json" } }).then(readJson);
  ...
  async function handleCreateAccess(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await fetch("/api/admin/access", {
        ...
        credentials: "include",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ userId: Number(accessForm.userId), projectId: Number(accessForm.projectId), accessLevel: accessForm.accessLevel }),
      }).then(readJson);
  ...
  async function handleDeleteAccess(accessId: number) {
    ...
    try {
      await fetch(`/api/admin/access/${accessId}`, { method: "DELETE", credentials: "include", headers: { Accept: "application/json" } }).then(readJson);
  ...
  if (loading) {
    return <main className="min-h-screen bg-[#0f172a] text-white flex items-center justify-center"><p className="animate-pulse">Carregando painel administrativo...</p></main>;
  }
    ...
    <main className="min-h-screen bg-[#0f172a] text-white">
      <header className="border-b border-white/10 bg-white/5">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <Link className="rounded-xl border border-white/20 px-4 py-2 text-sm hover:bg-white/10" to="/projects">Meus Projetos</Link>
            <button onClick={handleLogout} className="rounded-xl border border-white/20 px-4 py-2 text-sm hover:bg-white/10">Sair</button>
          </div>
        </div>
      </header>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-white/60">Cadastre clientes e defina uma senha inicial de acesso.</p>
              </div>
              {isEditingUser && <button className="rounded-lg border border-white/20 px-3 py-2 text-sm hover:bg-white/10" type="button" onClick={resetUserForm}>Cancelar edição</button>}
            </div>

            <form onSubmit={handleSaveUser} className="mt-5 grid gap-4 md:grid-cols-2">
              <label className="block"><span className="text-sm text-white/75">Nome</span><input className="mt-1 w-full rounded-xl border border-white/15 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-400" style={fieldStyle} value={userForm.name} onChange={(event) => setUserForm((current) => ({ ...current, name: event.target.value }))} /></label>
              <label className="block"><span className="text-sm text-white/75">E-mail</span><input className="mt-1 w-full rounded-xl border border-white/15 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-400" style={fieldStyle} type="email" value={userForm.email} onChange={(event) => setUserForm((current) => ({ ...current, email: event.target.value }))} required /></label>
              <label className="block"><span className="text-sm text-white/75">Perfil</span><select className="mt-1 w-full rounded-xl border border-white/15 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-400" style={selectStyle} value={userForm.role} onChange={(event) => setUserForm((current) => ({ ...current, role: event.target.value }))}><option value="client">client</option><option value="viewer">viewer</option><option value="editor">editor</option><option value="admin">admin</option></select></label>
              <label className="block"><span className="text-sm text-white/75">Status</span><select className="mt-1 w-full rounded-xl border border-white/15 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-400" style={selectStyle} value={userForm.active ? "active" : "inactive"} onChange={(event) => setUserForm((current) => ({ ...current, active: event.target.value === "active" }))}><option value="active">Ativo</option><option value="inactive">Inativo</option></select></label>
              <label className="block md:col-span-2"><span className="text-sm text-white/75">{isEditingUser ? "Nova senha opcional" : "Senha inicial"}</span><div className="mt-1 flex gap-2"><input className="w-full rounded-xl border border-white/15 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-400" style={fieldStyle} type="text" value={userForm.password} onChange={(event) => setUserForm((current) => ({ ...current, password: event.target.value }))} required={!isEditingUser} /><button className="rounded-xl border border-white/20 px-4 py-2 text-sm hover:bg-white/10" type="button" onClick={handleGeneratePassword}>Gerar</button></div>{lastGeneratedPassword && <p className="mt-2 rounded-lg border border-yellow-300/30 bg-yellow-500/10 px-3 py-2 text-xs text-yellow-100">Senha gerada: <strong>{lastGeneratedPassword}</strong>. Copie agora para enviar ao cliente.</p>}</label>
              <div className="flex flex-wrap gap-3 md:col-span-2"><button className="rounded-xl bg-blue-500 px-5 py-3 font-semibold text-white hover:bg-blue-400 disabled:opacity-60" type="submit" disabled={savingUser}>{savingUser ? "Salvando..." : isEditingUser ? "Salvar usuário" : "Criar usuário"}</button>{isEditingUser && <button className="rounded-xl border border-white/20 px-5 py-3 font-semibold text-white hover:bg-white/10" type="button" onClick={resetUserForm}>Limpar usuário</button>}</div>
            </form>
          </section>
          ...
          <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-white/60">Cadastre um projeto apontando para uma pasta e um JSON no Dropbox.</p>
              </div>
            ...
            <form onSubmit={handleSaveProject} className="mt-5 grid gap-4 md:grid-cols-2">
              ...
              {!isEditingProject && <label className="block md:col-span-2"><span className="text-sm text-white/75">Upload do JSON do Kepler</span><input className="mt-1 w-full rounded-xl border border-white/15 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-400" style={fieldStyle} type="file" accept=".json,application/json" onChange={handleProjectUploadFile} /><p className="mt-2 text-xs text-white/60">Ao enviar o JSON aqui, o backend cria a pasta no Dropbox, salva o arquivo e cadastra o projeto no D1 automaticamente.</p>{projectUploadFile && <p className="mt-2 rounded-lg border border-emerald-300/30 bg-emerald-500/10 px-3 py-2 text-xs text-emerald-100">Arquivo selecionado: <strong>{projectUploadFile.name}</strong></p>}</label>}
              ...
              <div className="flex flex-wrap gap-3 md:col-span-2"><button className="rounded-xl bg-blue-500 px-5 py-3 font-semibold text-white hover:bg-blue-400 disabled:opacity-60" type="submit" disabled={savingProject}>{savingProject ? "Salvando..." : isEditingProject ? "Salvar alterações" : projectUploadFile ? "Criar projeto e enviar JSON" : "Criar projeto"}</button>{isEditingProject && <button className="rounded-xl border border-white/20 px-5 py-3 font-semibold text-white hover:bg-white/10" type="button" onClick={resetProjectForm}>Limpar formulário</button>}</div>
            </form>
