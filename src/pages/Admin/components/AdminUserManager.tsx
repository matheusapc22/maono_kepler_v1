...
import AdminOrganizationAccessPanel from "./AdminOrganizationAccessPanel";
  ...
  useEffect(() => {
    ...
    if (isSuperAdmin) {
      fetch(`/api/admin/users/${selected.id}/organizations`, {
        ...
        .then(json)
        .then((data) => setMemberships(data.organizations || []))
        .catch((error) => onMessage("error", error.message));
    }
    ...
    <section className="admin-user-manager">
      <header>
        <div>
          ...
          <p>
            Crie usuários, atribua organizações e configure acessos ou delegação
            limitada.
          </p>
        </div>
        <button
      ...
      {creating && (
        <div className="admin-user-modal" role="dialog" aria-modal="true">
          <div className="admin-user-dialog compact">
            <header>
              <h3>Novo usuário</h3>
              <button onClick={() => setCreating(false)} aria-label="Fechar">×</button>
            </header>
            <form onSubmit={create}>
              ...
              <footer>
                <span />
                <span />
                <button type="button" className="mm-btn" onClick={() => setCreating(false)}>Cancelar</button>
                <button className="mm-btn primary" disabled={busy}>Criar usuário</button>
              </footer>
            </form>
          </div>
        </div>
