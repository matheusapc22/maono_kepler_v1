...
test("harness mede os estágios exigidos e não contém política de bloqueio", async () => {
  ...
  assert.match(source, /observedWebglCanvases/);
  assert.match(source, /webglContextLostCount: run\.webglContextLostCount/);
  assert.match(source, /WEBGL_CONTEXT_LOST_DURING_BENCHMARK/);
