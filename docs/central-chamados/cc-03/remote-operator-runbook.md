...
**Fonte vazia confirmada:** o inventário posterior mostrou zero legados elegíveis nas seis organizações. Use [reconcile-empty](reconcile-empty.md) para certificar essa situação, inclusive em organizações sem membros. Esse modo exige super admin e guarda atômica de fonte zero, não aceita fallback e nunca importa chamados. Os passos com `apply` abaixo continuam destinados à operação que pode importar legados.
