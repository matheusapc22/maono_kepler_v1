...
function createFileMetadata(row) {
  ...
  const revisionTime = Number.isFinite(
...
export function isLocalStorageMode(env) {
  ...
  const driver = String(env?.STORAGE_DRIVER || "")
    .trim()
    .toLowerCase();

  return (
    appEnv === LOCAL_APP_ENV &&
    driver === LOCAL_STORAGE_DRIVER
  );
}

export async function ensureLocalStorageFolder(
  env,
  path,
) {
  getDb(env);
...
export async function downloadLocalStorageFile(
  env,
  rootPath,
  fileName,
) {
  ...
  return new Response(
    storedContentToBytes(row.content),
    {
      status: 200,
      headers: {
        "Content-Type":
...
export async function getLocalStorageMetadata(
  env,
  rootPath,
  fileName,
) {
...
export async function listLocalStorageFolder(
  env,
  path = "",
) {
  const db = getDb(env);
  ...
  for (const row of result?.results || []) {
    const relativePath = String(row.path).slice(
      prefix.length,
    );
    ...
    if (separatorIndex >= 0) {
      const folderName = relativePath.slice(
        0,
        separatorIndex,
      );
      ...
      if (!folders.has(folderPath)) {
        ...
        entries.push(
          createFolderMetadata(folderPath),
        );
      }
  ...
  entries.sort((left, right) =>
    String(left.path_display).localeCompare(
      String(right.path_display),
    ),
  );
...
export async function deleteLocalStoragePath(
  env,
  path,
) {
  const db = getDb(env);
  ...
  if (!normalizedPath) {
    const error = new Error(
      "Não é permitido excluir a raiz do armazenamento local.",
    );
    error.status = 400;
